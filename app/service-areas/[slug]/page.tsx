...
export default async function LocationDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <>
      ...
      <section className="section shell"><div className="split-section"><div><SectionHeading eyebrow="Nearby Project Proof" title={`Cool Fox work near ${location.city}`} text="Approved project photos tagged to this community and nearby service areas will appear here without changing the page layout." /><Link className="button button-gold" href={`/contact?city=${location.slug}`}>Schedule in {location.city}</Link></div><ProjectPlaceholder compact /></div></section>
